...
input(inputPars *par, image *img){
   ...
   */
  par->radius                   = 2000*AU;
  par->minScale                 = 2*AU; // 2 * sizex / Nx / 2
  par->pIntensity               = 5000; 
  par->sinkPoints               = 1000; 
  par->dust                     = "../../lime/example_sf3dmodels/opacities_k05_230GHz_B_1_7.tab";
  par->moldatfile[0]            = "e-ch3oh.dat";
  ...
  par->collPartIds[0]           = CP_p_H2; //Coll part
  ...
  */
  ...
  img[i].imgres                 = 16e-3;           // Resolution in arc seconds
  ...
  img[i].filename               = "img_ch3oh_lte_on.fits"; //Output file
  img[i].freq                   = 241700219000.00; 
  ...
  img[i].freq                   = 241700219000.00;         //Continuum central frequency                                     
...
density(double dummy0, double dummy1, double id, double *density){
  int id_int=ceil(id);
  density[0] = sf3d->dens_p_H2[id_int]; 
}
